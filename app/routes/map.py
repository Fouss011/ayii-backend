...
async def cta_incidents(
    request: Request,
    status: str = Query("", description="new|confirmed|resolved (ignoré pour l’instant)"),
    limit: int = Query(20, ge=1, le=200),
    db: AsyncSession = Depends(get_db),
):
    # auth simple comme ton dashboard
    admin_tok = (os.getenv("ADMIN_TOKEN") or "").strip()
    req_tok = (request.headers.get("x-admin-token") or "").strip()
    if admin_tok and req_tok != admin_tok:
        raise HTTPException(401, "invalid admin token")

    # on prend les derniers reports 'cut' autour du monde
