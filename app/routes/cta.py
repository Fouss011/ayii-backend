...
# -----------------------------
# V2 MINIMAL (sans photo) — OK, phone confirmé
# -----------------------------
...
async def cta_incidents_v2(
    ...
    for r in rows:
        ...
        items.append({
            ...
            "status": m["status"],
            "photo_url": m["photo_url"],   # NULL ici, normal
...
@router.get("/incidents_pro")
async def cta_incidents_pro(
    request: Request,
    status: str = Query("", description="new|confirmed|resolved"),
    limit: int = Query(20, ge=1, le=200),
    db: AsyncSession = Depends(get_db),
    ...
    for r in rows:
        ...
        items.append({
            ...
            "created_at": m["created_at"],
            "status": m["status"],
            "photo_url": m["photo_url"],   # ✅ image OU vidéo (même champ)
            "age_min": int(m["age_min"]) if m["age_min"] is not None else None,
            "phone": m.get("phone"),       # ✅ téléphone
        })
    return {"api_version": "pro", "items": items, "count": len(items)}
