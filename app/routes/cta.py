...
async def cta_incidents(
    ...
    # SQL: on renvoie phone + une vignette si dispo
    q = text("""
        WITH latest_att AS (
          SELECT DISTINCT ON (
            kind,
            round(CAST(ST_Y(geom::geometry) AS numeric), 5),
            round(CAST(ST_X(geom::geometry) AS numeric), 5)
          )
            kind,
    ...
    for r in res.fetchall():
        items.append({
            ...
            "status": r.status,            # 'new'
            "photo_url": r.photo_url,      # image/vidéo si trouvée
            "age_min": int(r.age_min) if r.age_min is not None else None,
            "phone": getattr(r, "phone", None),  # 👈 téléphone exposé
        })
